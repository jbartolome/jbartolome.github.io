    ...
    <xsl:for-each select="stock:doc/stock:item">
        <xsl:if test="./stock:price &gt; 70">   
            ...
            <tr style="background-color: {$altRow}">
                ...
                <td style="border: 1px #757575 solid;"><xsl:value-of select="format-number(./stock:price,'$#.00##')"/></td>
...
<h3>The total of stocks that cost more than $70.00 is <xsl:value-of select="count(stock:doc/stock:item[./stock:price &gt; 70])"/></h3>
